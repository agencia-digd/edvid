...
4. **Ask → confirm → execute → iterate → persist.** Never touch the cut until the user confirms the strategy in plain English. A video arriving on Telegram is **not** a start signal — wait until she names the file and asks to edit. She sends other videos there.
...
└── edit/
    ...
    ├── final-social.mp4         ← encode leve pra IG/WhatsApp (`encode_social.py`)
...
the track reference you load after the gate. Two that also live here:

- **`caption_qa.py --transcript transcripts/cut.json --captions remotion/public/captions.json --write`** — glossary rewrite (DGD→dig.D, área→arte) + optional `--drop-sequence` for leftover phrases after a recut. **Mandatory** before `caption_style.py`. Default glossary: `assets/brand/digd/glossary.json`.
- **`encode_social.py <edit>/final.mp4`** — writes `final-social.mp4` (1080×1920, crf 21, aac 128k, bt709/tv). Always after `final.mp4`.
- **`pick_bed.py --mood <tense|punch|warm|cta|dark|tech> -o remotion/public/trilha.mp3`** — trilha local Mixkit quando não há Treblo. `musicAI` ligado sem chave **não** entrega mudo.
...
Both tracks: scaffold with one `cp -R` of the template, describe the video in `public/edit-data.json`, verify with montage stills, render, loudnorm, deliver `edit/final.mp4`, then `encode_social.py`. Load the `remotion-best-practices` skill when writing any Remotion code (CustomGraphics).

On a dig.D talking-head: read `assets/brand/digd/BRAND.md` before generating any split image, and run `caption_qa.py --write` after transcribing the cut. Do **not** start an edit because a video landed on Telegram — wait for the ask.
...
- Starting an edit because a video arrived on Telegram. She sends other videos there. Wait until she names the file and says to edit.
- Shipping captions that still say `DGD` / `área` / a phrase she already cut. `caption_qa.py` exists for this.
- Putting a split insert on the CTA beat. The last ask (`toca aqui`, `fala com a gente`) is full-frame or an endcard.
