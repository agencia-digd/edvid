...
| `elements.musicAI` | Phase 3: Treblo se houver chave; senão `pick_bed.py`. OFF → só voz |
...
2. **Generate machine data into `remotion/public/`:**
   ...
   - **`caption_qa.py` (mandatory).** Whisper invents brand names and
     near-homophones. Rewrite before any caption style runs:
     ```bash
     uv run python helpers/caption_qa.py \
       ...
       --write
     ```
     Add `--drop-sequence "Que a DGD não é"` when a recut left a phrase in the
     transcript that the picture no longer says. Default glossary is
     `assets/brand/digd/glossary.json`. Then regenerate captions from the
     rewritten transcript if you did not pass `--captions` (the helper writes
   ...
   - **Caption style** — from the Estilo tab pick. `stacked` ALSO needs
     ...
     (run AFTER caption_qa so EMPH/replacements land) plus
     `captions.style:"stacked"` (see the "Caption style" section).
...
5. **Render:** `npx remotion render Reels out/render.mp4`, then loudnorm →
   `edit/final.mp4` (see Phase 3). Then
   `uv run python helpers/encode_social.py <edit>/final.mp4` → `final-social.mp4`.
...
**On a dig.D video, read `assets/brand/digd/BRAND.md` first.** Illustrate the
claim, not the noun. Forbidden: hoodie/hacker as "sistema", oil-painting atelier
as "arte", lifestyle matcha as "posts". Generate landscape 16:9 with the subject
in the lower-center (split `top` crops to ~1080×750).
...
**CTA does not carry a split insert.** The last spoken ask (`Quer saber` /
`toca aqui` / `fala com a gente`) is full-frame or an endcard. End the last
`splitInserts` window on the cut *before* the CTA beat. Measured: 6s of the
wrong image on the closer kills the ask.

## Phase 3 — soundtrack (short-form)

Ask: **AI-generated** (Treblo) or **local file** (copy to `public/trilha.mp3`).

**If `musicAI` is ON and there is no `TREBLO_API_KEY`, do not skip the
soundtrack.** Pick from the Mixkit library shipped in `assets/music/`:

```bash
uv run python helpers/pick_bed.py --list
uv run python helpers/pick_bed.py --mood tech -o public/trilha.mp3
```

Moods: `tense` (gancho/segredo) · `punch` (energia) · `warm` (tutorial) ·
`cta` (pedido) · `dark` (mistério) · `tech` (sistema/operação/agência).
Choose from the cut transcript, not at random. License is Mixkit Free
(`assets/music/LICENSE.md`) — commercial OK, no attribution required, do not
republish the MP3s as a music pack. Then flip `soundtrack.enabled: true` and
mix at the usual `0.0445`.
...
- Generating split images without reading the brand pack (`assets/brand/digd/BRAND.md` on a dig.D video). "sistema" is operação, not a hoodie in the dark.
- Leaving a split insert on the CTA beat.
- Shipping `captions.json` without `caption_qa.py`. Whisper will write `DGD` and `área`.
- Starting an edit because a video arrived on Telegram. Wait for the ask.

---

## Helpers de Fase 2/3

- **`captions_for_remotion.py`** (karaoke JSON) · **`caption_qa.py`** (glossary rewrite + drop leftover phrases; mandatory before stacked/karaoke) · **`face_track.py`** (eye-track JSON) · **`person_matte.py`** (RVM alpha matte; `uv sync --extra matting`) · **`pexels_search.py`** · **`wikimedia_images.py`** (no key, brands/people first choice) · **`google_images.py`** (fallback, mind rights) · **`captions_srt.py`** (longform .srt) · **`chapters.py`** (YouTube chapters) · **`treblo_music.py`** (AI soundtrack) · **`pick_bed.py`** (Mixkit local: `--mood tense|punch|warm|cta|dark|tech`) · **`encode_social.py`** (`final.mp4` → `final-social.mp4`).
...
| `TREBLO_API_KEY` | trilha sonora composta por IA (Fase 3) | `pick_bed.py` na biblioteca Mixkit (`assets/music/`) |
